...
import com.cyl.musiclake.databinding.FloatControlViewBinding
...
class FloatControlView(context: Context) : FrameLayout(context) {
    ...
    private var binding: FloatControlViewBinding =
        FloatControlViewBinding.inflate(LayoutInflater.from(context), this, true)

     init {
        viewWidth = binding.floatView.layoutParams.width
        viewHeight = binding.floatView.layoutParams.height
    ...
    fun updateViewStatus(params: WindowManager.LayoutParams) {
        ...
        //上下两个按钮的状态值
        val topWidth = binding.powerSavingIv.width
...
//        if (playerViewY <= topHeight && playerViewX < maxRangeX && playerViewX + playerViewWidth > minRangeX) {
        if (playerViewY <= topHeight / 2) {
            if (!topSelected) {
                topSelected = true
                binding.powerSavingIv.isPressed = true
            }
...
//        } else if (playerY <= topHeight && playerViewX < maxRangeX && playerViewX + playerViewWidth > minRangeX) {
            ...
            if (!bottomSelected) {
                bottomSelected = true
                binding.clearIv.isPressed = true
            }
        } else {
            if (bottomSelected) {
                bottomSelected = false
                binding.clearIv.isPressed = false
            }
            if (topSelected) {
                topSelected = false
                binding.powerSavingIv.isPressed = false
            }
        }
