...
import com.cyl.musiclake.databinding.ActivityImportPlaylistBinding
...
class ImportPlaylistActivity : BaseActivity<BasePresenter<BaseContract.BaseView>>() {
    ...
    private val viewBinding by lazy { ActivityImportPlaylistBinding.inflate(layoutInflater) }
    ...
    override fun initData() {
        if (Intent.ACTION_SEND == intent.action && intent.type != null) {
            if (Constants.TEXT_PLAIN == intent.type) {
                ...
                viewBinding.playlistInputView.editText?.setText(title)
            }
        }
    }
    ...
    override fun listener() {
        super.listener()
        //同步歌单
        viewBinding.syncBtn.setOnClickListener {
            showLoading(true)
            val playlistLink = viewBinding.playlistInputView.editText?.text.toString()
            getPlaylistId(playlistLink)
        }
        //导入歌单
        viewBinding.importBtn.setOnClickListener {
            if (name == null) {
    ...
    fun showResultAdapter(result: Playlist) {
        ...
        viewBinding.resultRsv.adapter = mAdapter
        viewBinding.resultRsv.layoutManager = androidx.recyclerview.widget.LinearLayoutManager(this, androidx.recyclerview.widget.LinearLayoutManager.VERTICAL, false)
