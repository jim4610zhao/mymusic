...
class PlaylistDetailActivity : BaseActivity<PlaylistDetailPresenter>(), PlaylistDetailContract.View {
    ...
    private val viewBinding by lazy { FragPlaylistDetailBinding.inflate(layoutInflater) }
    ...
    override fun showCover(cover: String) {
        CoverLoader.loadImageView(context, coverUrl, viewBinding.albumArt)
    }
    ...
    override fun setToolbarTitle(): String? {
        ...
        mPlaylist?.let {
            ...
            CoverLoader.loadBigImageView(context, coverUrl, it.type, viewBinding.albumArt)
        }
        mArtist?.let {
            title = it.name
    ...
    override fun initView() {
        ...
        viewBinding.recyclerView.addItemDecoration(ItemDecoration(this, ItemDecoration.VERTICAL_LIST))
        viewBinding.recyclerView.adapter = mAdapter
        viewBinding.fab.setOnClickListener { PlayManager.play(0, musicList, pid) }
    }
...
//                mAdapter?.notifyDataSetChanged()
    ...
    override fun showPlaylistSongs(songList: MutableList<Music>?) {
        ...
        if (coverUrl == null) {
            ...
            CoverLoader.loadBigImageView(context, coverUrl, type, viewBinding.albumArt)
        }
        if (musicList.size == 0) {
            showEmptyState()
        }
