...
import com.cyl.musiclake.databinding.ActivityChartPlaylistBinding
...
abstract class BasePlaylistActivity : BaseActivity<PlaylistPresenter>(), PlaylistContract.View {
    ...
    var TOTAL_COUNTER = 0
    val limit = 10
    private val viewBinding by lazy { ActivityChartPlaylistBinding.inflate(layoutInflater) }
    ...
    private fun showHeaderInfo(playlist: Playlist?) {
        if (playlist != null) {
            CoverLoader.loadBitmap(this, playlist.coverUrl) {
                viewBinding.coverIv.setImageBitmap(it)
                viewBinding.coverBgIv.setImageDrawable(CoverLoader.createBlurredImageFromBitmap(it))
            }
            //接口未返回date，暂时注释
//            if (playlist.date != 0L) {
...
//            }
            playlist.des?.let {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    viewBinding.descTv.text = Html.fromHtml(playlist.des, Html.FROM_HTML_MODE_LEGACY)
                } else {
                    viewBinding.descTv.text = Html.fromHtml(playlist.des)
                }
            }
        }
    }
