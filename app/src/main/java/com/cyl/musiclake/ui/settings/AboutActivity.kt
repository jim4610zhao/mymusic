...
import com.cyl.musiclake.databinding.ActivityAboutBinding
...
class AboutActivity : BaseActivity<BasePresenter<BaseContract.BaseView>>() {
    ...
    override fun initView() {
        ...
        viewBinding.aboutContainerView?.startAnimation(animation1)
    }

    override fun initData() {
        viewBinding.cardAbout1.tvAboutVersion?.text = getString(R.string.about_version, BuildConfig.VERSION_NAME)
//        realTimeUserTv?.text = MusicApp.socketManager.realUsersNum.toString()
    ...
    private fun revealRed() {
        // 保存最开始的状态的参数
        val saveParams = viewBinding.shareFab!!.layoutParams
        ...
        transition.addListener(object : Transition.TransitionListener {
            ...
            override fun onTransitionEnd(transition: Transition) {
                animateRevealColor(viewBinding.aboutContainerView.findViewById(R.id.ll_layout), R.color.colorAccent)
                // 动画结束之后，将 红圈再设回以前的参数
                viewBinding.shareFab!!.layoutParams = saveParams
        ...
        TransitionManager.beginDelayedTransition(viewBinding.aboutContainerView.findViewById(R.id.ll_layout), transition)
    }
    ...
    fun onRealTimeEvent(event: SocketOnlineEvent) {
        viewBinding.cardAbout2.realTimeUserTv!!.text = event.num.toString()
    }
